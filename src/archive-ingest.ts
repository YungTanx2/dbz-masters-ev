...
// Only ingest Dragon Ball Super Card Game MASTERS prices (category 27).
const TARGET_CATEGORY_IDS = new Set(['27']);
...
export async function ingestDate(dateStr: string): Promise<void> {
  ...
  try {
    ...
    // 3. Walk extracted files, filter to DBS Masters (category 27), parse + ingest
    let filesIngested = 0;
    ...
    pruneOldRows(30);
    console.log(`[archive-ingest] Ingested ${dateStr}: ${filesIngested} DBS Masters sets`);
  } finally {
