 ...
 * DBS Masters rarity buckets are naturally homogeneous in subType after non-booster
 * filtering (Common/Uncommon = Normal, Rare/SR/SPR/SLR/Concept/SCR/GDR = Foil), so
 * grouping by rarity-only correctly captures all relevant cards without explicit
 * subType filtering.
 */
...
// All DBS Masters rarity buckets — used to seed byRarity with zeroed stats.
// Unlike One Piece/Digimon, there are no synthetic suffix-derived variant buckets —
// TCGCSV's extendedData.Rarity already carries the true rarity for every product
// (see tcgcsv.ts resolveRarity() for the verification note).
...
// Empty for DBS Masters: every rarity in ALL_RARITIES has an estimated pull rate in
// pull-rates.default.json, so CASE_HIT_THRESHOLD alone (via hitDistribution fraction)
// correctly classifies Secret Rare / God Rare as case hits — no "unknown rate" buckets
// exist here the way Digimon's Textured/Signed/Gold Border/Rare Pull variants did.
const ALWAYS_CASE_HIT = new Set<string>([]);

/**
 * Calculate expected value (EV) for a DBS Masters booster box.
 *
 * Pack structure (commonCount is always defined for the 4 supported Standard sets):
 ...
 *   hitDistribution entries  (Super Rare, Special Rare, Special Leader Rare,
 *                             Concept Rare, Secret Rare, God Rare)
 *
